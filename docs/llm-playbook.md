...
4. Enforce temporal data rules (datetime with timezone offset for all temporal values).
5. Validate with tests/build.
6. Update docs when architecture/process changed.
...
- Introducing date-only fields or offset-less datetime strings.
...
## Temporal Standard (Required)
- Do not introduce date-only storage for business temporal data.
- Model temporal fields as PocketBase `date` and persist RFC3339 datetime with timezone offset (`Z` is valid).
- If UI uses `datetime-local`, convert it to an offset-aware datetime before sending to PocketBase.
