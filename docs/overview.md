...
- Accept public enrollment requests from `/register`:
  ...
  - submit flow creates a `users` auth record with role `father`, then creates `fathers`, `students`, and `students_fathers`,
  - new father records created from public registration persist `fathers.user_id` pointing to the created `users` record,
...
- Manage student-family data in PocketBase:
  ...
  - optional `user_id` relation links a father to a `users` auth record,
...
- Manage employee administrative reports in PocketBase:
  - `employee_reports` collection stores the relation between employee and semester plus a job snapshot,
  - required relations: `employee_id`, `semester_id`,
  - required snapshot fields: `job_name`, `job_salary`,
  ...
  - create uses an `employee_jobs` dropdown but persists the selected job name and salary directly on the report,
  - edit shows `job_name` and `job_salary` as read-only snapshot fields,
  - table default order is `created_at` descending,
...
- Manage administrative event email messaging:
  ...
  - actual delivery runs through server functions in `src/lib/server/email/`,
  - delivery uses SMTP credentials from the app runtime environment (`SMTP_USER`, `SMTP_PASSWORD`, `SMTP_HOST`, `SMTP_PORT`, `SMTP_FROM`).
...
- Server-side event email messaging requires:
  - `SMTP_USER`
  - `SMTP_PASSWORD`
  - `SMTP_HOST`
  - `SMTP_PORT`
