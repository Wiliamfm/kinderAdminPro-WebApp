...
export const getGuardiansByGrade = server$(async function (gradeId: number) {
  ...
  const { data: guardians, error: guardiansError } = await getSupabase().from("guardians_students").select("guardians(*)").in("student_id", data.map(s => s.id));
  if (guardiansError) {
    console.error(`Unable to get guardians:\n`, guardiansError);
    return [];
  }
  ...
  const uniqueGuardians = Array.from(
    new Map(guardians.map(gs => [gs.guardians.id, gs.guardians])).values()
  );

  return uniqueGuardians.map(guardian => {
    return {
      id: guardian.id,
      name: guardian.name,
      documentNumber: guardian.document_number,
