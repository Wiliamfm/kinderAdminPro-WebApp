...
import { listBulletinsStudentsForExport } from '../../pocketbase/bulletins-students';
...
function sortBulletinStudentsForExport(records: BulletinStudentRecord[]): BulletinStudentRecord[] {
...
function groupBulletinStudentsForExport(
...
function createStudentsExportPdf(records: BulletinStudentRecord[], generatedAt: Date = new Date()): jsPDF {
  const doc = new jsPDF({
...
export type StudentsExportResult = {
  fileName: string;
  data: string;
  mimeType: string;
};

export async function exportStudentsReport(options: StudentsExportOptions = {}): Promise<StudentsExportResult> {
  ...
  const fileName = `reportes_estudiantes_${year}${month}${day}_${hour}${minute}.pdf`;
