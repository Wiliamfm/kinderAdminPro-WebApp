...
export const useCreateEmployeeLeave = routeAction$(
  async (data, event) => {
...
export default component$(() => {
  ...
  const employees = employeesLoader.value.employees.map(
    (e: EmployeeResponse) => {
      return {
        id: e.id,
        name: e.name,
        ...
        actions: [
          ...
          <button
            class="cursor-pointer"
            data-modal-target="leavesModal"
            data-modal-toggle="leavesModal"
            onClick$={async () => {
          ...
          <button
            class="cursor-pointer"
            data-modal-target="invoicesModal"
            data-modal-toggle="invoicesModal"
            onClick$={async () => {
              selectedEmployee.value = e.id;
              employeeInvoices.invoices.length = 0;
              employeeInvoices.invoices = await getEmployeeInvoices({
                employeeId: e.id,
              });
    ...
    <div class="flex h-full flex-col place-items-center space-y-10">
      ...
      <div class="flex">
        <FormModal
          ...
          formAction={createEmployeeAction}
          formOnSubmitFn={employeeFormFn}
        >
          <div>
            <label
              for="name"
        ...
        <FormModal
          modalId="employee-leave-form"
          modalTitle={"Registrar Incapacidad"}
          modalBtnName={"Registrar Incapacidad"}
          modalBtnClass="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-red-200 via-red-300 to-yellow-200 group-hover:from-red-200 group-hover:via-red-300 group-hover:to-yellow-200 dark:text-white dark:hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-red-100 dark:focus:ring-red-400"
          formBtnName="Registrar incapacidad"
          formBtnClass="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
          formAction={createEmployeeLeaveAction}
          formOnSubmitFn={employeeLeaveFormFn}
        >
      ...
      >
        <div class="relative max-h-full w-full max-w-2xl">
          ...
          <div class="relative rounded-lg bg-white shadow-sm dark:bg-gray-700">
            ...
            {/*<!-- Modal footer -->*/}
            <div class="flex items-center space-x-2 rounded-b border-t border-gray-200 p-6 rtl:space-x-reverse dark:border-gray-600"></div>
          </div>
        </div>
      </div>
        ...
        <div class="relative max-h-full w-full max-w-2xl">
          ...
          <div class="relative rounded-lg bg-white shadow-sm dark:bg-gray-700">
            ...
            <div class="flex items-start justify-between rounded-t border-b p-5 dark:border-gray-600">
              ...
              <button
                type="button"
                class="group relative ms-auto me-2 mb-2 inline-flex items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-red-200 via-red-300 to-yellow-200 p-0.5 text-xs font-medium text-gray-900 group-hover:from-red-200 group-hover:via-red-300 group-hover:to-yellow-200 focus:ring-4 focus:ring-red-100 focus:outline-none dark:text-white dark:hover:text-gray-900 dark:focus:ring-red-400"
                onClick$={() => {
                  showEmployeeInvoiceForm.value =
                    !showEmployeeInvoiceForm.value;
                }}
              >
            ...
            <div class="space-y-6 p-6">
              {showEmployeeInvoiceForm.value && (
                <Form
                  action={createEmployeeInvoiceAction}
                  onSubmitCompleted$={async (_, element) => {
                    ...
                    element.reset();
                    employeeInvoicesTableProps.data = await getEmployeeInvoices(
                      {
                        employeeId:
                          createEmployeeInvoiceAction.value.employeeInvoice
                            .employeeId,
                      },
                    );
                    alert(
                      `Factura "${createEmployeeInvoiceAction.value.employeeInvoice.fileName}" registrada correctamente`,
                    );
                  }}
                >
                  <input
