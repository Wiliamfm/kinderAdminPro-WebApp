...
export default component$(() => {
  ...
  const gradesTable = gradesLoader.value.map(j => {
    return {
      ...j, actions: [
        <a href={`/payroll/grades/${j.id}`} class="cursor-pointer">
  ...
  return (
    <div class="flex flex-col place-items-center h-full space-y-10">
      ...
      <FormModal modalId="job-form-modal" modalTitle={"Agregar Curso"} modalBtnName={"Agregar Curso"} modalBtnClass="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-red-200 via-red-300 to-yellow-200 group-hover:from-red-200 group-hover:via-red-300 group-hover:to-yellow-200 dark:text-white dark:hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-red-100 dark:focus:ring-red-400" formBtnName="Crear Curso" formBtnClass="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" formAction={createGradeAction} formOnSubmitFn={$((_: unknown, element: HTMLFormElement) => {
