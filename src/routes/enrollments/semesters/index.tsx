...
export default component$(() => {
  ...
  const gradesTable = semestersLoader.value.map((semester) => {
    return {
      ...
      actions: [
        <a
          href={`/enrollments/semesters/${semester.id}`}
    ...
    <div class="flex h-full flex-col place-items-center space-y-10">
      ...
      <FormModal
        ...
        formOnSubmitFn={$((_: unknown, element: HTMLFormElement) => {
          if (createSemesterAction.value?.failed) {
            if (createSemesterAction.value.message) {
              alert(createSemesterAction.value.message);
            } else if (createSemesterAction.value.fieldErrors) {
              alert(createSemesterAction.value.fieldErrors.name);
            } else {
